...
// Usernames that are always admins (case-insensitive). Can still set others in Config/Admin.
const ALWAYS_ADMIN_USERNAMES = ['peliy11', 'admin'];

function ensureAlwaysAdmin(users) {
  return users.map((u) => {
    const lower = (u.username || '').toLowerCase();
    const isAlwaysAdmin = ALWAYS_ADMIN_USERNAMES.includes(lower);
    return isAlwaysAdmin ? { ...u, isAdmin: true } : u;
  });
}
